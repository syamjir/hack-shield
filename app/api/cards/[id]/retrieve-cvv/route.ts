...
export async function GET(
  ...
  try {
    ...
    // 🔐 Decrypt the Card
    const plainCardCvv = await cardDoc.getDecryptedCVV();

    // create mongoose document object
    const docObject = cardDoc.toObject();
    docObject.cvv = plainCardCvv;
    const newData = docObject;
    return NextResponse.json(
      {
        message: "Secret card-cvv retrieved successfully",
        data: newData,
      },
      { status: 200 }
    );
  } catch (err) {
    console.error("Retrieve card-cvv error:", err);
    return NextResponse.json(
