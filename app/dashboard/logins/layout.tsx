...
export default function IdentitiesLayout({ children }: CardsLayoutProps) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const pathname = usePathname();

  const tabs: Tab[] = [
    { name: "logins", href: "/dashboard/logins" },
    { name: "Bin", href: "/dashboard/logins/bin" },
  ];
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center ">
        <div>
          <h2 className="text-2xl font-bold text-primary-a20">Logins</h2>
          <p className="text-sm text-dark-a0/50">
            Manage your saved logins details.
          </p>
        </div>
        <Button
      ...
      {/* Identity form modal */}
      <LoginForm isOpen={isModalOpen} onOpenChange={setIsModalOpen} />
    </div>
  );
}
